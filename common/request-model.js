...
export default ({
      ...
      if (new Model() instanceof LinkParent) {
        if (typeof acceptedHook === 'function') {
          ...
          throw new Meteor.Error(
            'notAFunction',
            'Second parameter of onAccepted must be a function'
          )
        }
      } else {
        throw new Meteor.Error(
          'notALinkParent',
          'First parameter of onAccepted must be a model of type LinkParent'
        )
      }
    }
  ...
  if (Meteor.isServer) {
    RequestsCollection.createIndexAsync({
      requesterId: 1,
      invitedId: 1,
      type: 1
    })
    RequestsCollection.createIndexAsync({ createdAt: -1 })
  }
